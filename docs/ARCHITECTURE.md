...
- A bracket is used as a **personal assist tool** on the vote board (not the primary persisted vote model)
...
3. User can optionally run **Bracket Assist** locally to seed ordering.
...
5. Server upserts votes and sets `participant.submittedAt`.

Result visibility:
- Consensus uses all persisted `TierVote` rows
- Individual vote list in results shows participants with `submittedAt != null`
...
Key paths:
- `src/app/sessions/*`
  - `join`, `new`, `[sessionId]`, `[sessionId]/vote`, `[sessionId]/results`
- `src/components/tierlist/TierListBoard.tsx`
  - main vote UX, draft save, bracket assist, submit
